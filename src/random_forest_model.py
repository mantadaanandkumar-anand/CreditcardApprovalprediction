# ==============================================================================
# Credit Card Approval Prediction System - Random Forest Model
# ==============================================================================
...
def train_random_forest(X_train: pd.DataFrame, y_train: pd.Series, preprocessor) -> Pipeline:
    """
    Creates and trains a pipeline combining the preprocessor and a Random Forest classifier.
    """
    print("\n--- Training Random Forest Model ---")
    
    # Instantiate Random Forest with standard parameters
    rf = RandomForestClassifier(n_estimators=220, max_depth=9, class_weight='balanced', random_state=42)
...
def evaluate_model(pipeline: Pipeline, X_test: pd.DataFrame, y_test: pd.Series) -> dict:
    ...
    plt.tight_layout()
    cm_path = figures_dir / 'rf_confusion_matrix.png'
    ...
    plt.figure(figsize=(6, 5))
    plt.plot(fpr, tpr, color='darkred', lw=2, label=f'ROC curve (AUC = {metrics["roc_auc"]:.2f})')
    ...
    plt.tight_layout()
    roc_path = figures_dir / 'rf_roc_curve.png'
...
def display_feature_importance(pipeline: Pipeline, X_train: pd.DataFrame):
    """
    Retrieves, lists, and visualizes the feature importances of the random forest model.
    """
    ...
    sns.barplot(data=top_features, x='Importance', y='Feature', hue='Feature', palette='plasma', legend=False)
    plt.title('Random Forest Feature Importances')
    plt.xlabel('Relative Importance Score')
    plt.ylabel('Features')
    plt.tight_layout()
    imp_path = figures_dir / 'rf_feature_importance.png'
...
def main():
    ...
    # Train the pipeline
    model_pipeline = train_random_forest(X_train, y_train, preprocessor)
    ...
    print(f"Saved trained Random Forest Pipeline to: {model_path}")
