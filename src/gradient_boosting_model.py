# ==============================================================================
# Credit Card Approval Prediction System - Gradient Boosting Model
# ==============================================================================
...
def train_gradient_boosting(X_train: pd.DataFrame, y_train: pd.Series, preprocessor) -> Pipeline:
    """
    Creates and trains a pipeline combining the preprocessor and a Gradient Boosting classifier.
    
    Args:
        X_train (pd.DataFrame): Training feature data.
        y_train (pd.Series): Training target labels.
        preprocessor (ColumnTransformer): Fitted preprocessor configuration.
        
    Returns:
        Pipeline: Trained scikit-learn pipeline object containing preprocessor and model.
    """
    print("\n--- Training Gradient Boosting Model ---")
    
    # Instantiate Gradient Boosting Classifier with production parameters
...
def evaluate_model(pipeline: Pipeline, X_test: pd.DataFrame, y_test: pd.Series) -> dict:
    ...
    Args:
        pipeline (Pipeline): Trained classifier pipeline.
        X_test (pd.DataFrame): Test feature data.
        y_test (pd.Series): Test target labels.
        
    Returns:
        dict: Set of calculated performance metrics.
    """
    ...
    plt.tight_layout()
    cm_path = figures_dir / 'gb_confusion_matrix.png'
    ...
    plt.figure(figsize=(6, 5))
    plt.plot(fpr, tpr, color='indigo', lw=2, label=f'ROC curve (AUC = {metrics["roc_auc"]:.2f})')
    ...
    plt.tight_layout()
    roc_path = figures_dir / 'gb_roc_curve.png'
...
def display_feature_importance(pipeline: Pipeline, X_train: pd.DataFrame):
    """
    Retrieves, lists, and visualizes the feature importances of the Gradient Boosting model.
    
    Args:
        pipeline (Pipeline): Trained classifier pipeline.
        X_train (pd.DataFrame): Dataframe used for training to map raw inputs.
    """
    ...
    sns.barplot(data=top_features, x='Importance', y='Feature', hue='Feature', palette='magma', legend=False)
    plt.title('Gradient Boosting Feature Importances')
    plt.xlabel('Relative Importance Score')
    plt.ylabel('Features')
    plt.tight_layout()
    imp_path = figures_dir / 'gb_feature_importance.png'
...
def main():
    ...
    # Train the pipeline
    model_pipeline = train_gradient_boosting(X_train, y_train, preprocessor)
    ...
    print(f"Saved trained Gradient Boosting Pipeline to: {model_path}")
